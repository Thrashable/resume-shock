...
export default function Landing({ onDataReady }) {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    setError('')
    try {
      // Lazy-load the PDF engine (~2 MB) only when a real PDF is parsed, so the
      // landing page and the "See Example" demo never download it.
      const { extractTextFromPDF, parseResume } = await import('../utils/resumeParser')
