...
export default function Preview({ data, profileImage, onReset }) {
  ...
  const handleCopyLink = useCallback(async () => {
    ...
    setSizeWarning(size > 50000
      ? 'Your share link is very long. For best results, use a smaller profile picture.'
      : '')

    const url = generateShareURL(shareData)
    setShareUrl(url) // always show the link so the feature never silently fails

    // Try the modern Clipboard API, then a legacy fallback. Either way the link
    // is visible above for manual copy (works in any browser / insecure context).
    let ok = false
    try {
  ...
  // If someone lands on /preview directly (e.g. a refresh) with no data in
  // memory, send them home cleanly instead of navigating during render.
  if (!data) {
  ...
  return (
    ...
    >
      ...
      {shareUrl && (
        <div className="max-w-6xl mx-auto px-4 mt-3">
          <div className="p-2.5 rounded-lg bg-accent/5 border border-accent/20 flex items-center gap-2">
            <span className="text-xs text-accent font-mono shrink-0">{copied ? '✓ Copied!' : 'Your link:'}</span>
            <input
              readOnly
              value={shareUrl}
              onFocus={(e) => e.target.select()}
              aria-label="Shareable resume link"
              className="flex-1 bg-transparent text-xs text-gray-400 font-mono outline-none truncate"
            />
          </div>
        </div>
      )}
